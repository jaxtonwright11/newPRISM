...
export async function GET(request: Request) {
  ...
  const communityNotifications = buildCommunityNotificationPayloads(newPerspectives);

  let totalSent = 0;
  for (const notification of communityNotifications) {
    const sent = await sendPushToCommunityFollowers(notification.communityId, {
  ...
  return NextResponse.json({
    ...
    communities: communityNotifications.length,
  });
}
