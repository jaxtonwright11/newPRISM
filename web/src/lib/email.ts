...
function sanitizeColor(value: string): string {
  return /^#[0-9a-fA-F]{6}$/.test(value) ? value : "#D4956B";
}
...
export async function sendDigestEmail(
  ...
  const comparisonTopicTitle = comparison ? escapeHtml(comparison.topicTitle) : "";
  const comparisonTopicSlug = comparison ? encodeURIComponent(comparison.topicSlug) : "";
  const comparisonSubject = comparison
    ? sanitizeSubject(comparison.topicTitle)
  ...
  const comparisonHtml = comparison ? `
    <div style="margin: 24px 0; padding: 20px; border-radius: 12px; background: #181B20; border: 1px solid #262A31;">
      ...
      <h2 style="font-size: 16px; font-weight: 700; margin: 0 0 16px 0; color: #EDEDEF;">${comparisonTopicTitle}</h2>
      ${comparison.perspectives.map(p => `
        <div style="padding: 12px 0 12px 16px; border-left: 3px solid ${sanitizeColor(p.color)}; margin-bottom: 12px;">
      ...
      `).join("")}
      <a href="${appUrl}/compare/${comparisonTopicSlug}" style="display: inline-block; margin-top: 8px; color: #D4956B; text-decoration: none; font-size: 13px; font-weight: 500;">
  ...
  try {
    await client.emails.send({
      ...
      to,
      subject: comparison
        ? `This week on PRISM: ${comparisonSubject}`
        : "Your PRISM Weekly Digest",
      html: `
