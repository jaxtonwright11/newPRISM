...
function safeHexColor(value: string | null | undefined): string {
  return /^#[0-9A-Fa-f]{6}$/.test(value ?? "") ? value as string : "#D4956B";
}
...
export async function sendDigestEmail(
  ...
  const appUrl = (process.env.NEXT_PUBLIC_SITE_URL ?? "https://web-liard-psi-12.vercel.app").replace(/\/+$/, "");
  const feedUrl = escapeHtml(`${appUrl}/feed`);
  ...
  const comparisonHtml = comparison ? `
    <div style="margin: 24px 0; padding: 20px; border-radius: 12px; background: #181B20; border: 1px solid #262A31;">
      ...
      ${comparison.perspectives.map(p => `
        <div style="padding: 12px 0 12px 16px; border-left: 3px solid ${safeHexColor(p.color)}; margin-bottom: 12px;">
      ...
      `).join("")}
      <a href="${escapeHtml(`${appUrl}/compare/${encodeURIComponent(comparison.topicSlug)}`)}" style="display: inline-block; margin-top: 8px; color: #D4956B; text-decoration: none; font-size: 13px; font-weight: 500;">
        See the full comparison &rarr;
      </a>
    </div>
  ` : "";
  const digestHtml = escapeHtml(digest).replace(/\n/g, "<br>");
  ...
  try {
    await client.emails.send({
      ...
      to,
      subject: comparison
        ? `This week on PRISM: ${sanitizeSubject(comparison.topicTitle)}`
