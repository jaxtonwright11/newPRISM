...
const DEFAULT_COMPARISON_COLOR = "#D4956B";
...
function safeComparisonColor(color: string): string {
  return /^#[0-9A-Fa-f]{6}$/.test(color) ? color : DEFAULT_COMPARISON_COLOR;
}

export function buildDigestEmailSubject(comparison?: DigestComparison | null): string {
  return comparison
    ? sanitizeEmailSubject(`This week on PRISM: ${comparison.topicTitle}`)
...
export function buildDigestEmailHtml(
  ...
  const feedUrl = escapeHtml(`${baseUrl}/feed`);
  const settingsUrl = escapeHtml(`${baseUrl}/settings`);
  ...
  const comparisonHtml = comparison ? `
    <div style="margin: 24px 0; padding: 20px; border-radius: 12px; background: #181B20; border: 1px solid #262A31;">
      ...
      ${comparison.perspectives.map(p => `
        <div style="padding: 12px 0 12px 16px; border-left: 3px solid ${safeComparisonColor(p.color)}; margin-bottom: 12px;">
      ...
      `).join("")}
      <a href="${escapeHtml(`${baseUrl}/compare/${encodeURIComponent(comparison.topicSlug)}`)}" style="display: inline-block; margin-top: 8px; color: #D4956B; text-decoration: none; font-size: 13px; font-weight: 500;">
  ...
  return `
        <div style="font-family: 'DM Sans', sans-serif; max-width: 600px; margin: 0 auto; background: #0F1114; color: #EDEDEF; padding: 32px;">
          ...
          <div style="margin-top: 32px; padding-top: 16px; border-top: 1px solid #262A31;">
            <a href="${feedUrl}" style="color: #D4956B; text-decoration: none; font-size: 14px; font-weight: 500;">
              Open PRISM &rarr;
            </a>
          </div>
          <p style="font-size: 11px; color: #5C6370; margin-top: 24px;">
            ...
            <a href="${settingsUrl}" style="color: #5C6370; text-decoration: underline;">Unsubscribe</a>
          </p>
        </div>
      `;
}
...
export async function sendDigestEmail(
  ...
  try {
    await client.emails.send({
      ...
      to,
      subject: buildDigestEmailSubject(comparison),
      html: buildDigestEmailHtml(digest, appUrl, comparison),
    });
    return { success: true };
  } catch (err) {
