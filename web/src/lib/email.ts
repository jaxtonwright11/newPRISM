...
function getAppUrl(): string {
  return (process.env.NEXT_PUBLIC_SITE_URL ?? "https://web-liard-psi-12.vercel.app").replace(/\/+$/, "");
}
...
export async function sendDigestEmail(
  ...
  const comparisonHtml = comparison ? `
    <div style="margin: 24px 0; padding: 20px; border-radius: 12px; background: #181B20; border: 1px solid #262A31;">
      ...
      `).join("")}
      <a href="${escapedAppUrl}/compare/${encodeURIComponent(comparison.topicSlug)}" style="display: inline-block; margin-top: 8px; color: #D4956B; text-decoration: none; font-size: 13px; font-weight: 500;">
      ...
      html: `
        <div style="font-family: 'DM Sans', sans-serif; max-width: 600px; margin: 0 auto; background: #0F1114; color: #EDEDEF; padding: 32px;">
          ...
          <div style="margin-top: 32px; padding-top: 16px; border-top: 1px solid #262A31;">
            <a href="${escapedAppUrl}/feed" style="color: #D4956B; text-decoration: none; font-size: 14px; font-weight: 500;">
              Open PRISM &rarr;
            </a>
          </div>
          <p style="font-size: 11px; color: #5C6370; margin-top: 24px;">
            ...
            <a href="${escapedAppUrl}/settings" style="color: #5C6370; text-decoration: underline;">Unsubscribe</a>
          </p>
        </div>
      `,
    });
