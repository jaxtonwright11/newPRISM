...
export function buildDigestEmailHtml(
  ...
  const safeDigest = escapeHtml(digest).replace(/\n/g, "<br>");
  const comparisonUrl = comparison
    ? `${appUrl}/compare/${encodeURIComponent(comparison.topicSlug)}`
    : "";
  ...
  const comparisonHtml = comparison ? `
    <div style="margin: 24px 0; padding: 20px; border-radius: 12px; background: #181B20; border: 1px solid #262A31;">
      ...
      ${comparison.perspectives.map((perspective) => `
        <div style="padding: 12px 0 12px 16px; border-left: 3px solid ${perspective.color}; margin-bottom: 12px;">
          <p style="font-size: 11px; font-weight: 600; color: #9CA3AF; margin: 0 0 4px 0;">${escapeHtml(perspective.communityName)}</p>
          <p style="font-size: 14px; color: #EDEDEF; margin: 0; line-height: 1.5;">&ldquo;${escapeHtml(perspective.quote)}&rdquo;</p>
        </div>
      `).join("")}
      <a href="${comparisonUrl}" style="display: inline-block; margin-top: 8px; color: #D4956B; text-decoration: none; font-size: 13px; font-weight: 500;">
