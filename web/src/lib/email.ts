...
function safeCommunityColor(color: string): string {
...
export async function sendDigestEmail(
  ...
  const escapedDigest = escapeHtml(digest).replace(/\n/g, "<br>");
  ...
  const comparisonHtml = comparison ? `
    <div style="margin: 24px 0; padding: 20px; border-radius: 12px; background: #181B20; border: 1px solid #262A31;">
      ...
      ${comparison.perspectives.map(p => `
        <div style="padding: 12px 0 12px 16px; border-left: 3px solid ${safeCommunityColor(p.color)}; margin-bottom: 12px;">
  ...
  try {
    await client.emails.send({
      ...
      to,
      subject: comparison
        ? sanitizeSubject(`This week on PRISM: ${comparison.topicTitle}`)
