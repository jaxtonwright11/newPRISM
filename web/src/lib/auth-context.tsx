...
const supabaseNotConfiguredError = () => new AuthError('Supabase is not configured');
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const signUp = async (email: string, password: string, username: string) => {
    if (!supabase) return { error: supabaseNotConfiguredError(), confirmationRequired: false };
  ...
  const signIn = async (email: string, password: string) => {
    if (!supabase) return { error: supabaseNotConfiguredError() };
