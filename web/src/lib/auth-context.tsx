...
function authNotConfiguredError(): AuthError {
  return new AuthError('Authentication is not configured');
}
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const signUp = async (email: string, password: string, username: string) => {
    if (!supabase) return { error: authNotConfiguredError(), confirmationRequired: false };
  ...
  const signIn = async (email: string, password: string) => {
    if (!supabase) return { error: authNotConfiguredError() };
