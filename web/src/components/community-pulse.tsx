...
export function CommunityPulse({ isOpen, onClose }: CommunityPulseProps) {
  ...
  const newCommunities = SEED_COMMUNITIES.filter(
    (c) =>
      c.community_type === "cultural" || c.community_type === "diaspora"
  ).slice(0, 3);
      ...
      <div className="relative w-full max-w-sm bg-prism-bg-secondary border border-prism-border rounded-2xl shadow-2xl animate-slide-in-right overflow-hidden mt-12 md:mt-0">
        ...
        <div className="p-4 space-y-4 max-h-[70vh] overflow-y-auto">
          ...
          >
            <div
              ...
              style={{
                ...
                borderLeftColor:
                  COMMUNITY_COLORS[
                    mostReacted.community.community_type as CommunityType
                  ],
              }}
            >
          ...
          <PulseSection
            icon={
              <svg
                className="w-4 h-4 text-prism-community-diaspora"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                strokeWidth={2}
              ...
              {newCommunities.map((c) => (
                ...
                >
                  <div
                    ...
                    style={{
                      backgroundColor:
                        COMMUNITY_COLORS[c.community_type] + "20",
          ...
          {/* Engagement stat */}
          <div className="p-3 rounded-lg bg-gradient-to-br from-prism-accent-active/10 to-prism-community-diaspora/10 border border-prism-accent-active/20">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-prism-text-dim mb-1">
              Today&apos;s alignment highlight
            </p>
            <p className="text-sm text-prism-text-primary font-medium">
              93% of communities agree: worker retraining must be funded before
              plant closures
            </p>
            <p className="text-[10px] text-prism-text-dim mt-1.5">
              3 communities • EV Transition
            </p>
          </div>
        </div>
      </div>
